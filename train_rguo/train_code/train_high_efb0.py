...
class Trainer():
    def __init__(self, model, options):
        ...
        meta0=dict()
        for i in range(5):
            with open(SETTINGS["LEVEL0_JSON_DIR"]+"mode0/Attention_fold{}.json".format(i),"r") as f:
                meta0.update(json.load(f))

        meta1=dict()
        for i in range(5):
            with open(SETTINGS["LEVEL0_JSON_DIR"]+"mode1/Attention_fold{}.json".format(i),"r") as f:
                meta1.update(json.load(f))

        meta2=dict()
        for i in range(5):
            with open(SETTINGS["LEVEL0_JSON_DIR"]+"mode2/Attention_fold{}.json".format(i),"r") as f:
                meta2.update(json.load(f))

        with open(SETTINGS["LEVEL0_JSON_DIR"]+"Validation_mode.json", "r") as f:
            valid_mode=json.load(f)

        set_gpu_environ()
        train_dataset = PANDA_Dataset_Attention(train_df,
        ...
        valid_dataset = PANDA_Dataset_Attention(valid_df,
                                                options['train_data_path'],
                                                [meta0,meta1],
                                                valid=valid_mode,
                                                num_patches=options['num_patches'],
    ...
    def train(self, epoch):
        ...
        for i, (image,label,patch_label,ignore_mask,pesudo_label,pesudo_prob,provider) in enumerate(tbar):
            ...
            #print(reg_label)
            #print(provider)
            loss_reg = F.mse_loss(reg_output,wt*label.float()+(1-wt)*pesudo_label.float().cuda())
            ...
            #print(total_loss_reg)
            #print(total_loss_reg/(i+1))
            total_loss_cls+=loss_cls.item()
            total_loss_patch+=loss_patch.item()

            loss=loss_reg+loss_cls+loss_patch
            ...
            del loss,loss_reg,loss_cls,loss_patch,reg_output,cls_output,patch_pred,A
    ...
    def start_train(self):
        for epoch in range(self.start_epoch, self.max_epoch):
            for i in range(len(self.optimizer.param_groups)):
                    self.optimizer.param_groups[i]['lr'] = 0.0001 * np.cos(np.pi * epoch / self.max_epoch) + 0.0001
            self.train(epoch)
            self.valid(epoch)
...
def main(fold):
    options=dict()
    options['train_data_path'] = SETTINGS["LEVEL0_TILE_DIR"]  #path to tiles
    #options['train_mask_path'] = "../level1/mask/" #deprecated
    ...
    options['lr'] = 0.0002         #deprecated
    options['lr_step'] = (8,15,20) #deprecated
    options['lr_decay_ratio']=0.25 #deprecated

    options['resume_path']=SETTINGS["LEVEL1_WEIGHTS_RGUO"]+"efficientnet-b0_fold{}_bestOptimQWK.pth".format(fold) #resume from last level resolution

    options['arch']="efficientnet-b0"
    options['model_name']="{}_fold{}".format(options['arch'],fold)
    #options['save_dir']="./tmp/"
    options['save_dir'] = SETTINGS["LEVELHALF_WEIGHTS_RGUO"]+"/{}_regression_levelhalf_attention_smooth_cosine_largebs_fold{}/".format(options['arch'],fold)
    options['dropout']=0.4
    options['regression']=True

    options['patch_size']=384
    options['num_patches']=36
    ...
    from models.utils import set_momentum
    model=set_momentum(model,0.01)
    print(model)
