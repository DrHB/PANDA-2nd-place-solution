...
model_urls = {
    ...
    'resnet34_seblock': 'https://download.pytorch.org/models/resnet34-333f7ec4.pth',
...
class SE_Block(nn.Module):
    def __init__(self,num_patch,channels,reduction=4):
    ...
    def forward(self,x):
        _,c,h,w=x.shape
        x=x.view(-1,self.num_patch,c,h,w) #b,n,c,h,w
        module_input = x #b,n,c,h,w
        #squeeze
        x=torch.mean(x,dim=(1,3,4)) #b,c
        x = self.fc1(x)
        x = self.relu(x)
        x = self.fc2(x)
        x = self.sigmoid(x)#b,c
        x=x.view(x.size(0),1,x.size(1),1,1)
        #print("excitation",x.shape)
        x=module_input * x
        return x.view(-1,c,h,w)


class ResNet_SE(nn.Module):
    def __init__(self, model,num_patch=48):
        ...
        self.se2=SE_Block(num_patch,self.layer2[-1].bn2.num_features)
        self.layer3 = model.layer3
        self.se3=SE_Block(num_patch,self.layer3[-1].bn2.num_features)
        self.layer4 = model.layer4
        self.se4=SE_Block(num_patch,self.layer4[-1].bn2.num_features)
...
def resnet34_seblock(num_classes=1000, pretrained='imagenet',num_patch=48):
    """Constructs a ResNet-34 model.
    """
    model = models.resnet34(pretrained=False, num_classes=num_classes)
    if pretrained is not None:
        settings = pretrained_settings['resnet34'][pretrained]
        model = load_pretrained(model, num_classes, settings)
    model = ResNet_SE(model,num_patch)
    return model
